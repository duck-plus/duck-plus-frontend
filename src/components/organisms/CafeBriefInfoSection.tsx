  ...
  return (
    <div>
      ...
      <CafeBriefInfo>
        {/* 간략 주소 */}
        <DetailItem>
          <LocationSVGR width={hr * 16} height={hr * 16} fill={theme.colors.gray800} />
          {cafe?.address.briefAddress}
        </DetailItem>
        {/* SNS */}
        {cafe?.snsList.filter(isNonNullable).map(({ type, channelName, url }) => {
          const SNSIcon =
            type === 'KAKAO' ? KakaoSVGR : type === 'INSTAGRAM' ? InstagramSVGR : TwitterSVGR;

          return (
            <DetailItem key={`${channelName}_${url}`}>
              <SNSIcon width={hr * 16} height={hr * 16} fill={theme.colors.gray800} />
              <SNSChannelName
                onClick={() => {
                  ga.send('sns_btn', {
                    type,
        ...
        {/* 운영 시간 */}
        <DetailItem>
          <TimeSVGR width={hr * 16} height={hr * 16} fill={theme.colors.gray800} />
          {/* 요일 */}
          {cafe?.businessHour.businessDayList.length === 7
            ...
            : cafe?.businessHour.businessDayList
                ...
                .reduce(
                  (prev, day, idx) => `${prev}${idx > 0 ? ' ' : ''}${dayToLocaleStringMap[day]}`,
                  ''
                ) + ' '}
          {/* 시간 */}
          {cafe?.businessHour.openingTime}~{cafe?.businessHour.closingTime}
          {/* 휴일에도 운영 하면 휴무없음 딱지*/}
          {cafe?.businessHour.workingOnHoliday ? <NoHolidayBadge>휴무없음</NoHolidayBadge> : null}
        </DetailItem>
      </CafeBriefInfo>
    </div>
  );
